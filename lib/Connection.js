...
Connection.prototype.sendVersion = function() {
  ...
  put.pad(4); // procurrency expects addresses with timestamp
  put.pad(26); // addr_me
  put.pad(4); // procurrency expects addresses with timestamp
  put.pad(26); // addr_you
  put.put(nonce);
  ...
  put.word32le(0);
  //put.word32le(1); //advertise as full client
...
Connection.prototype.sendBlock = function(block, txs) {
  ...
  this.sendMessage('mblk', put.buffer());
};
...
Connection.prototype.parseMessage = function(command, payload) {
  ...
  switch (command) {
    case 'version': // https://en.bitcoin.it/wiki/Protocol_specification#version
      ...
      data.addr_me = parser.buffer(30);  //procurrency sends addresses with timestamp in version message
      data.addr_you = parser.buffer(30); //
